...
// Delay
    ...
    u_delay(
      ...
      .dout({vcount_delay,hcount_delay,vsync_delay,vblnk_delay,hsync_delay,hblnk_delay,rgb_delay})
    );
    ...
    u_delay_2(
      .clk(s00_axi_aclk),
      .rst(!s00_axi_aresetn),
      .din({vcount_delay,hcount_delay,vsync_delay,vblnk_delay,hsync_delay,hblnk_delay,char_line}),
      .dout({vcount_out,hcount_out,vsync_out,vblnk_out,hsync_out,hblnk_out,char_line_delay})
    );
// User logic   
    wire [2:0] pixel;
    reg [11:0] rgb_out_nxt;
     ...
     if(!s00_axi_aresetn) begin
      ...
      always @* begin
           if(char_pixels[3'b111-pixel] == 1 && hblnk_delay == 0 && vblnk_delay == 0 && vcount_delay < ypos + ((TEXT_HEIGHT*16)<<scale) && vcount_delay >= ypos && hcount_delay < xpos + ((TEXT_WIDTH*8)<<scale) && hcount_delay >= xpos) rgb_out_nxt = text_color;
